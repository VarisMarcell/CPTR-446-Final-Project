...
let SongsService = class SongsService {
    ...
    findOne(id) {
        const song = this.em.findOneOrFail(song_entity_1.Song, { id: id });
        return song;
    }
    async update(id, updateSongDto) {
        ...
        if (updateSongDto.playlists) {
            for (let playlist of updateSongDto.playlists) {
                song.playlists.add(this.em.getReference(playlist_entity_1.Playlist, playlist));
            }
        }
        await this.em.persistAndFlush(song);
        return song;
